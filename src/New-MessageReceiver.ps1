...
(
	...
	[string] $MessageFactory = (Get-Variable -Name $MyInvocation.MyCommand.Module.PrivateData.MODULEVAR -ValueOnly).MessageFactory
	, 
...
try 
{
	...
	[Microsoft.ServiceBus.Messaging.MessageReceiver] $MessageReceiverClient = $null;
	...
	$OutputParameter = $MessageReceiverClient;
	$fReturn = $true;
